...
export const handler: Handler<StoreEmbeddingsEvent, StoreEmbeddingsResult> = async (event) => {
  const config = new Config()
  const aiServices = new AIServices(config)
  const storageServices = new StorageServices(config, config.vectorIndexName)

  const embeddingResult = await aiServices.getEmbeddingResult(event.embeddingsS3Uri)

  const clipEmbeddings = embeddingResult.data.filter(
    (item: MarengoEmbeddingItem) => item.embeddingScope === 'clip'
  )

  const segmentEmbeddings = clipEmbeddings.map((item: MarengoEmbeddingItem) => ({
