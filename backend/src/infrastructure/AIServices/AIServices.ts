...
export class AIServices implements EmbeddingGenerator, SegmentSelector {
  ...
  async startAsyncEmbedding(
    videoS3Uri: string,
    outputS3Uri: string,
  ): Promise<string> {
    if (!this.config.accountId) {
      throw new EmbeddingGenerationError(
        'AWS_ACCOUNT_ID environment variable is not configured',
      );
    }
    ...
    try {
      const response = await this.bedrockClient.send(
        new StartAsyncInvokeCommand(requestParams),
      );
  ...
  async analyzePegasusSync(
    ...
    try {
      ...
      const result = await this.invokePegasus(
        videoS3Uri,
        prompt,
        PEGASUS_ANALYSIS_JSON_SCHEMA,
      );
