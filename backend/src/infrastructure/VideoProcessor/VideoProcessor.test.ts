    ...
    const mockProcess = {
      stdout: {
        on: vi.fn((event, callback) => {
          if (event === 'data') callback(Buffer.from('60.5'));
        }),
      },
      stderr: {
        on: vi.fn(),
...
describe('VideoProcessor', () => {
  ...
  describe('buildFilter', () => {
    it('should build basic room label filter', () => {
      const filter = (videoProcessor as unknown as { buildFilter: (title: string) => string }).buildFilter('Living Room');
    ...
    it('should build intro filter with agency and street', () => {
      const filter = (videoProcessor as unknown as { buildFilter: (title: string, isIntro?: boolean, agencyLabel?: string, streetLabel?: string) => string }).buildFilter(
        'Exterior',
        true,
        'Skyline Estates',
        '123 Main Street'
  ...
  describe('sanitizeText', () => {
    it('should remove special characters', () => {
      const sanitize = (videoProcessor as unknown as { sanitizeText: (text: string) => string }).sanitizeText;
