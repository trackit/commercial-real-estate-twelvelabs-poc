  ...
  async execute(input: ProcessVideoInput): Promise<ProcessVideoOutput> {
    ...
    try {
      ...
      for (let i = 0; i < input.segments.length; i++) {
        ...
        await this.videoProcessor.processSegment(
          processingInput,
          segmentOutputPath,
        );
      ...
      await this.videoProcessor.concatenateSegments(
        segmentPaths,
        finalLocalPath,
      );
      ...
      await this.videoStorage.putObject(
        bucket,
        key,
        finalVideoData,
        'video/mp4',
      ...
      const totalDuration =
        await this.videoProcessor.getVideoDuration(finalLocalPath);
